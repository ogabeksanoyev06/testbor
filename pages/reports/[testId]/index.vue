<template>
  <div class="bg-white p-4 sm:rounded-xl">
        ...
        <UiTableBody>
          <UiTableRow v-for="(item, i) in data?.data" :key="i" class="odd:bg-muted">
            ...
            <UiTableCell class="font-medium">
              <NuxtLink class="text-primary" :to="localePath(`/reports/${route.params.testId}/${item.subject?._id}`)">{{ item.subject?.name_uz }}</NuxtLink>
            </UiTableCell>
...
<script setup>
  ...
  const { data } = await useAsyncData("ScienceReportResults", async () => {
